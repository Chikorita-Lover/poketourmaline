...
.continue
	...
	ldh a, [hSCY]
	push af
	call SubwayTrain_LoadGFX_PlayMusic
	ld hl, hVBlank
	ld a, [hl]
	push af
	ld [hl], 1
.loop
	...
	call SubwayTrain_Jumptable
	call SubwayTrain_UpdateLYOverrides
	call PushLYOverrides
	call DelayFrame
	jr .loop

.initialize
	call SubwayTrain_Jumptable_FirstRunThrough
	jr .loop
...
SubwayTrain_LoadGFX_PlayMusic:
	call ClearBGPalettes
	call ClearSprites
	call DisableLCD
	callfar ClearSpriteAnims
	call SetSubwayTrainPals
	call DrawSubwayTrain
	...
	call SubwayTrain_InitLYOverrides
...
.loop
	...
	hlbgcoord 0, 6
	ld de, SubwayTrainTilemap
	ld c, SCREEN_WIDTH
	call .FillLine
	hlbgcoord 0, 7
	ld de, SubwayTrainTilemap + SCREEN_WIDTH
	ld c, SCREEN_WIDTH
	call .FillLine
	hlbgcoord 0, 8
	ld de, SubwayTrainTilemap + (SCREEN_WIDTH * 2)
	ld c, SCREEN_WIDTH
	call .FillLine
	hlbgcoord 0, 9
	ld de, SubwayTrainTilemap + (SCREEN_WIDTH * 3)
	ld c, SCREEN_WIDTH
	call .FillLine
	ret
...
GetSubwayTrainBGTiles:
	push hl
	ld e, a
	ld d, 0
	ld hl, SubwayTrainBGTiles
	add hl, de
	add hl, de
	ld e, [hl]
	inc hl
...
SubwayTrain_Jumptable_FirstRunThrough:
...
.MenuData:
	...
	db 2, 12 ; rows, columns
	db SCROLLINGMENU_ITEMS_NORMAL ; item format
	dba .Destinations
	dba .PrintDestination
	dba NULL

.Destinations:
	db NUM_SUBWAY_DESTINATIONS
for x, NUM_SUBWAY_DESTINATIONS
	db x + 1
endr
	db -1
...
.PrintDestination:
	ld a, [wMenuSelection]
	call Subway_GetLocation
	ld a, [hl]
	push de
	ld [wNamedObjectIndex], a
	call GetSubwayDestinationName
	pop hl
	call PlaceString
	ret
...
INCLUDE "data/maps/subway_destinations.asm"
